...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("------------------------------------------------------------")
        self.stdout.write("CONTINUITY WATCHDOG METRICS:")
        self.stdout.write(f"  - ELIGIBLE_CYCLES_EXPECTED:          {state.eligible_cycles_expected}")
        self.stdout.write(f"  - ELIGIBLE_CYCLES_OBSERVED:           {state.eligible_cycles_observed}")
        self.stdout.write(f"  - ELIGIBLE_CYCLES_MISSING:            {state.eligible_cycles_missing}")
        self.stdout.write(f"  - DUPLICATE_NOOP_CYCLES:             {state.duplicate_noop_cycles}")
        self.stdout.write(f"  - EXPECTED_MARKET_CLOSURES:          {state.expected_market_closure_cycles}")
        self.stdout.write(f"  - LAST_EXPECTED_ELIGIBLE_CLOSE:      {state.last_expected_eligible_close.isoformat() if state.last_expected_eligible_close else 'None'}")
        self.stdout.write(f"  - LAST_PROCESSED_ELIGIBLE_CLOSE:     {state.last_processed_eligible_close.isoformat() if state.last_processed_eligible_close else 'None'}")
        self.stdout.write("------------------------------------------------------------")
