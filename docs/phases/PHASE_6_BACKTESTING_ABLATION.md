...
> **Status:** ✅ **APPROVED**  
> **Primary Goal:** Implement point-in-time historical backtesting, walk-forward time-series splitting with label purging and embargo, realistic trade friction simulation, and automated component ablation testing for XAUUSD.

---

## 1. Non-Negotiable Core Principle (R2 & A09)

> **ONE ENGINE RULE:** The backtesting engine must NEVER maintain a simplified secondary set of trading rules. Live analysis and backtesting must resolve the **exact same pure-Python `SignalEngine` class, version, configuration, and feature set**.

```text
HISTORICAL STORE (XAUUSD) ──> Build Point-in-Time MarketContext(t) ──> SignalEngine.analyze(context)
                                                                               │
                                                                               ▼
METRICS & ABLATION <── Record Trade <── Simulate Execution <── BUY / SELL CANDIDATE (if emitted)
```
...
- **Purging:** If signal outcome labeling horizon is 24 hours (96 bars on 15m), remove 96 bars immediately preceding train/validation/test boundaries to eliminate overlapping target leakage.
...
1. **Trigger:** `engine.analyze(t)` emits `BUY` or `SELL` candidate signal.
2. **Fill:** `EntryExecutionModel` calculates fill price on the next bar or post-signal quote.
3. **Frictions Applied:**
   - Bid-Ask Spread: Configured percentage or actual quote spread.
   - Maker/Taker Fees: Configured percentage.
   - Execution Slippage: Configured adverse slippage.
4. **Monitoring:** Side-aware triple-barrier evaluation (TP1, TP2, Stop Loss, or Max Holding Time Horizon).
5. **Intrabar Resolution:** Applies `IntrabarResolver` (1m/5m replay or `CONSERVATIVE_SL_FIRST`) on ambiguous candles.
...
| **Subsystem Breakdown** | Performance partitioned by **Regime**, **Session**, and **Direction (Long vs Short)** |
...
+ Macro USD Confirmation                            → PF = 1.95, Expectancy = +0.41R (+14%)
```
...
|---|---|---|
| **A09** | One Engine Parity | Live engine and backtester resolve identical `SignalEngine` class, version, and config. |
...
- [x] Backtest harness imports pure `SignalEngine` without code divergence.
...
- [x] Full metric suite calculated with regime/session/direction breakdowns.
