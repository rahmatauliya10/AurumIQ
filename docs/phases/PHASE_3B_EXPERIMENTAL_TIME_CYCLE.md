...
3. **Canonical Instrument Normalization (`normalize_target_instrument`):**  
   Strict canonical equivalence is enforced: `XAUUSD` and `XAU/USD` are accepted as canonical XAUUSD. Generic gold labels (`GOLD`, `XAU`, `GOLD_REFERENCE`) are never mapped to XAUUSD. Legacy aliases are restricted to `XAUT` and `XAUTUSD`.
4. **Causal Descriptive Operation & Strict Sample Quality:**  
   Uncalibrated XAUUSD computes pure descriptive mathematical facts. In the absence of an explicit `SampleEvaluation`, `SampleQuality` defaults strictly to `SampleQuality.INSUFFICIENT`, preventing any invented `MEDIUM` quality tier and ensuring `reliability_score` remains `0.0`.
5. **Deterministic 4-Stage Promotion Precedence (`evaluate_promotion_eligibility`):**  
...
6. **Closed-Candle & Time-Grid Isolation (PIT-Safe):**  
...
- In uncalibrated mode: returns descriptive ACF series and candidate dominant lag, but `is_significant = False` and `confidence_bound = 0.0`. Uses `acf_bartlett_z_multiplier` when configured without hardcoded 30/60/100 sample-tier leakage or invented quality tiers.
...
|---|---|:---:|---|:---:|
| **XAUUSD Acceptance** | `tests/acceptance/test_xauusd_phase3b.py` | 34 | Profile governance, zero defaults, canonical normalization, explicit sample quality, symmetric mismatch rejection, deterministic promotion precedence, closed-candle split, hostile lock, naive dt, runtime TF | ✅ PASS |
...
**Total Targeted Tests:** 68/68 passing (0 failures, 0 warnings).  
**Full Regression Suite:** 455/455 passing across whole repository.
