...
def test_end_to_end_ingestion_and_repository_pipeline():
    ...
    """
    # Explicitly register legacy BinanceProvider and UsdtUsdRateProvider within historical test scope
    if not registry.has("binance"):
        registry.register(BinanceProvider())
    if not registry.has("usdt_usd"):
